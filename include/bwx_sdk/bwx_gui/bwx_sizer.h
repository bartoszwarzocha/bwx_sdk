...
template <typename T>
concept bwxIsValidTypeForSizer =
    std::is_base_of_v<wxWindow, std::remove_pointer_t<T>> || std::is_base_of_v<wxSizer, std::remove_pointer_t<T>>;
...
public:
    ...
    wxSizerItem* AddWithLabel(wxWindow* w, const wxString& label, bool expand = true, bool l_to_r = false,
                              wxAlignment orient = wxALIGN_LEFT, int extra_flags = 0);

    wxSizerItem* AddButtons(wxWindow* btn1, wxWindow* btn2 = NULL, wxWindow* btn3 = NULL, wxWindow* btn4 = NULL,
                            wxOrientation orient = wxHORIZONTAL, bwxAlign align = wxALIGN_RIGHT,
                            int marg = bwxSIZER_DEFAULT_MARGIN);
...
#define bwxSetBoxSizerH(name, parent) \
    bwxBoxSizerH(name);               \
    parent->SetSizer(name)

#define bwxSetBoxSizerV(name, parent) \
    bwxBoxSizerV(name);               \
    parent->SetSizer(name)
