...
// Opcje: _NORMAL_BLOCK/_CLIENT_BLOCK
	...
	#define bwxMemStat(x) _CrtMemState _##x; \
...
#define _ptr_member(T, x, f) public: \
	void Set##f(T* x) { this->m_##x = x; } \
	T* Get##f() { return this->m_##x; } \
	private: T* m_##x

#define _member_isOk public: \
...
#define _static(Class, T, x, f) public: \
	...
	private: static T m_##x

#define _vector(T, x, f) public: \
	...
	private: wxVector<T> m_##x

#define _vector_inline(T, x, f) public: \
...
#define _vector_static(Class, T, x, f) public: \
	...
	private: static wxVector<T> m_##x

#define _std_vector(T, x, f) public: \
	void Add##f(T x) { this->m_##x.push_back(x); } \
	T Get##f(const int& index) { return this->m_##x[index]; } \
	int Get##f##Count() { return this->m_##x.size(); } \
	void Clear##f##Vector() { this->m_##x.clear(); } \
	...
	private: std::vector<T> m_##x

#define _std_vector_inline(T, x, f) public: \
	inline void Add##f(T x) { this->m_##x.push_back(x); } \
	inline T Get##f(const int& index) { return this->m_##x[index]; } \
	inline int Get##f##Count() { return this->m_##x.size(); } \
	inline void Clear##f##Vector() { this->m_##x.clear(); } \
...
#define _std_vector_static(Class, T, x, f) public: \
	static void Add##f(T x) { Class::m_##x.push_back(x); } \
	static T Get##f(const int& index) { return Class::m_##x[index]; } \
	static int Get##f##Count() { return Class::m_##x.size(); } \
	static void Clear##f##Vector() { Class::m_##x.clear(); } \
	...
	private: static std::vector<T> m_##x

#define _map(T1, T2, x, f) public: \
	...
	private: std::map<T1, T2> m_##x

#define _map_static(Class, T1, T2, x, f) public: \
	...
	private: static std::map<T1, T2> m_##x
